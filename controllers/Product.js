...
//           reviews =$(index).find("._3LWZlK").text();  
...
//           })
//         productData.push({image,title,price,ratings,originalPrice,specifications,offerPercentage,reviews})
//       })
...
try {
  ...
  if (sort[1]) {
    ...
    const products = await productSchema.find({ title : { $regex: search, $options: "i" } })
			.sort(sortBy)
			.skip(page * limit)
			.limit(limit);
